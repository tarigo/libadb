//! A device serving its pairing port, for driving `adb_pair`: the
//! server side of the exchange, blocking on its own thread over
//! `rustls`' own stream, driving the protocol by hand.
//!
//! It shares the SPAKE2 primitive with the client, the one piece that
//! cannot be written twice here; the core pins that primitive against
//! vectors of its own. Everything around it — the exporter, the
//! framing, the cipher, the `PeerInfo` — the two sides reach
//! independently.
...
use libadb::pairing::{Role, Spake2};
...
/// The GUID the fake device answers with.
pub const DEVICE_GUID: &str = "adb-fake-device-guid";

/// How far the device lets the exchange run.
#[derive(Clone, Copy)]
pub enum Conduct {
    /// Play its part to the end.
    Serve,
    /// Hang up after reading the host's SPAKE2 message, as a device
    /// whose dialog was dismissed would.
    QuitEarly,
}
...
/// The pairing server's throwaway certificate, minted once per test
/// binary rather than per run: what it proves does not depend on it
/// being fresh.
...
fn device_config() -> Arc<ServerConfig> {
...
pub fn spawn_pairing_device(code: &'static str) -> (String, JoinHandle<DeviceOutcome>) {
    spawn_pairing_device_that(code, Conduct::Serve)
}

/// The same, with a say in how far the exchange gets.
pub fn spawn_pairing_device_that(
    code: &'static str,
    conduct: Conduct,
) -> (String, JoinHandle<DeviceOutcome>) {
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let addr = listener.local_addr().unwrap().to_string();

    let handle = std::thread::spawn(move || {
        let (socket, _) = listener.accept().unwrap();
        let conn = ServerConnection::new(device_config()).unwrap();
        ...
        if let Conduct::QuitEarly = conduct {
            // Dropping the stream is the hang-up: no close_notify, as
            // a dialog dismissed mid-exchange gives none either.
            return outcome;
        }
