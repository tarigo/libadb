...
- Сопряжение `adb pair` не реализовано. Ключ, которому устройство уже
  доверяет — подтверждённый по USB, — принимается и по TLS, потому что
  adbd сверяет оба с одним хранилищем; ключ, которого устройство не
  видело, надо один раз спарить штатным клиентом.
