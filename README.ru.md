...
`Authenticator` — пользовательский трейт, но с фичей `keys` крейт даёт
готовую реализацию: `keys::AdbKey` разбирает ключ, который `adb` держит
в `~/.android/adbkey`, то есть переиспользует доверие устройства, либо
генерирует собственный — установленный `adb` при этом не нужен вовсе.
С фичей `host-keys` крейт ещё и сам находит и пишет эти файлы:

```rust,ignore
use libadb::keys::store;

// `rng` — любой `CryptoRngCore`: `OsRng` на хосте, аппаратный TRNG на
// микроконтроллере. При загрузке из него читают один раз — засеять
// генератор, которым ключ дальше ослепляет каждую подпись; генерация
// же черпает энтропию весь перебор простых, а потом ещё и на посев.
// И там и там заимствование заканчивается вместе с конструктором.
// Переиспользовать ~/.android/adbkey либо создать его — устройство
// один раз спросит подтверждение, ровно как и для adb.
let auth = store::load_or_generate(&dir, &mut rng, "user@host")?;
```

Каталог всегда передаётся параметром: библиотека не читает окружение и
не выбирает пути сама. Существующий ключ переиспользуется нетронутым, а
нечитаемый — это ошибка, а не повод затереть ключ, которому устройство,
возможно, уже доверяет.

Без `std` — на микроконтроллере — то же самое делает `keys::AdbKey`
напрямую, оставляя сохранение вам: `generate` принимает любой CSPRNG,
`to_pkcs8_der` вместе с `encode_key_record` укладывают ключ — с
контрольной суммой — в область флеша без файловой системы, а
`public_key_line()` даёт строку `adbkey.pub`. Крейты `rsa` и `zeroize`,
чьими типами говорит это API, реэкспортированы как
`libadb::keys::{rsa, zeroize}`: версию нужно согласовать одну, а не
угадывать её в своём манифесте.
...
| `reverse`     | `reverse:…`                  | Правила обратного проброса; входящие каналы — `accept_incoming` |

## Ограничения

- Беспроводная отладка Android 11+ — тот порт, который выдаёт `adb pair`
  и показывает панель «Беспроводная отладка», — требует TLS, которого
  этот крейт не знает. Устройство отвечает на рукопожатие пакетом `STLS`,
  и `connect` возвращает `ProtocolError::TlsRequired`. Подключайтесь по
  USB либо по открытому порту, который поднимает `adb tcpip 5555`.
