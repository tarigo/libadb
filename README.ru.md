...
- `OPEN` со стороны устройства не обрабатывается: обратные пробросы
  (`adb reverse`) и всё прочее, что устройство открывает к хосту, не
  обслуживаются. Каналы открывает только хост.
