...
// Либо сделать свой; устройство один раз спросит подтверждение.
let auth = AdbKey::generate(&mut rng, "user@host")?;
```

Сохранение ключа — за вызывающим: `to_pkcs8_pem` отдаёт содержимое
файла `adbkey`, `public_key_line()` — строку для `adbkey.pub`, а
записывать их на диск вызывающему. На микроконтроллере
`to_pkcs8_der` вместе с `encode_key_record` укладывают ключ — с
контрольной суммой — в область флеша, где нет файловой системы. Крейты
`rsa` и `zeroize`, чьими типами говорит это API, реэкспортированы как
