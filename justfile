...
lib_features := "tokio smol tokio,usb tokio,rusb tokio,smol tokio,nusb,rusb smol,usb smol,rusb tokio,host-keys smol,host-keys host-keys tokio,tls,host-keys smol,tls,host-keys"
...
doc_features := "tokio smol tokio,rusb smol,nusb tokio,host-keys tokio,tls,host-keys tokio,smol,nusb,rusb"
msrv_version := "1.87.0"
...
mutants_features := "tokio,smol,nusb,rusb,host-keys,tls"
...
coverage_features := "tokio,smol,host-keys,tls"
...
msrv:
    #!/usr/bin/env bash
    set -euo pipefail
    for f in tokio smol split keys host-keys tls tokio,rusb smol,nusb tokio,smol; do
        echo "== msrv libadb [$f]"
