...
ffi_features := "usb rusb nusb,rusb tls pairing pairing,usb"
...
mutants_features := "tokio,smol,nusb,rusb,host-keys,tls,pairing"
...
coverage_features := "tokio,smol,host-keys,tls,pairing"
...
msrv:
    #!/usr/bin/env bash
    set -euo pipefail
    for f in tokio smol split keys host-keys tls pairing tokio,rusb smol,nusb tokio,smol; do
        echo "== msrv libadb [$f]"
...
# Build the C examples against the cdylib. `ffi_pair` links only when
# the library has `pairing`, which also serves a wireless-debugging
# port, so that is the default; other feature sets build `ffi_shell`
# alone.
ffi-example features="pairing":
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    case "{{features}}" in
        *pairing*)
            cc -I libadb-ffi/include -o target/ffi_pair libadb-ffi/examples/ffi_pair.c \
                -L target/debug -ladb -lpthread -ldl -lm
            echo "built target/ffi_pair" ;;
    esac
